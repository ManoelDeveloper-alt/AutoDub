...
PATH_WORK = './' # pasta na qual estará os arquivos
...
def remover_silencio_final(caminho_entrada, caminho_saida=None, limiar_db=-50, min_duracao_ms=300):
    ...
    if not partes_nonsilenciosas:
        print("Áudio todo é considerado silêncio!")
        return
...
def voz_in_file(text, file_name):
    tts_audio_silent = PATH_WORK+"tts_audio_com_silencio.mp3"
    async def _interno():
        communicate = edge_tts.Communicate(text, voice="pt-BR-FranciscaNeural")
        await communicate.save(tts_audio_silent)

    asyncio.run(_interno())  # <- bloqueia até terminar
    remover_silencio_final(tts_audio_silent,file_name)
    os.remove(tts_audio_silent)
...
def text_to_speak(falas):    
    ...
    for i,fala in enumerate(falas): # uma vez para cada fala
        ...
        voz_in_file(text, temp_audio_tts_path)
